...
//     TERRAIN_GRASS,
//     TERRAIN_DIRT,
//     TERRAIN_PUDDLE // special, handled separately
// };
...
//     float driftAngle = 0.0f; // Visual drift effect
...
//         position.y = info.height + 0.5f; // update Y to match terrain
...
//             switch (info.type) {
//         case TERRAIN_ROAD:
//             speedMult = 1.3f;   // fastest
//             steerMult = 1.1f;
//             break;
//         case TERRAIN_GRASS:
//             speedMult = 0.6f;   // slower
//             steerMult = 0.8f;
//             break;
//         case TERRAIN_DIRT:
//             speedMult = 0.4f;   // slowest
//             steerMult = 0.6f;
//             break;
//         case TERRAIN_PUDDLE:
//             speedMult = 0.15f;  // extremely slow
//             steerMult = 0.2f;   // almost uncontrollable
//             drift = true;       // force drift in puddles
//             break;
//         }

//         // Steering sensitivity based on terrain
...
//             // Check if car's center is inside building's 2D footprint
...
// float getTerrainHeight(float x, float z) {
//     float height = 0;
//     float scale = 0.02f; // Gentler slopes
//     height += noise(x * scale, z * scale) * 5.0f;       // Base rolling hills
//     height += noise(x * scale * 2, z * scale * 2) * 2.0f; // Medium detail
//     height += noise(x * scale * 4, z * scale * 4) * 0.5f; // Fine detail
//     return height;
// }
...
// TerrainInfo getTerrainInfo(float x, float z) {
//     // ✅ Reuse existing height function
...
//     // Override with puddle if applicable
...
//     if (gameStarted && (w || s)) {
//     std::cout << "Car pos: (" << car.position.x << ", " << car.position.z << ")\n";
//     }
...
//     // Simple text rendering placeholder - you'd use a proper text library in production
...
//     float lastFrame = glfwGetTime();  // ← NOT 0.0f


//     while (!glfwWindowShouldClose(window)) {
//         float currentFrame = glfwGetTime();
//         deltaTime = currentFrame - lastFrame;
//         deltaTime = fminf(deltaTime, 0.1f);// Cap at 100ms to prevent huge jumps
...
//                     cop.position = car.position + glm::vec3(50, 0, 50); // Push away
//                     std::cout << "HIT BY POLICE! -5 seconds\n";
//                 }
//             }
            
//             // Police shoot bullets
